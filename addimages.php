...
$input_form = "<div class='form-group row'><div class='". $size . "'><div class='form-group'><label for='$input_id'>" . $input_label . "</label><input type='" . $input_type . "' class='form-control' name='" . $input_id . "' id='" . $input_id . "' placeholder='" . $input_label . "'></div></div></div>"; 
 ...
 echo "<div class='container-fluid'><form action='processimg.php' method='post'>";
 $input_labela = 'Title';
 $input_typea = 'text';
 ...
 echo "<textarea rows='20' cols='30' name='descp'></textarea></div><br />";
  echo "<br /><div class='form-group row'><div class='form-group'><label for='exampleSelect1'>Example select</label><select class='form-control' name='cate'>";
   
  
 require "conf.php";
         $stmt = $db->query("SELECT * FROM albumcategory");
 ...
 echo "</select></div></div><button class='btn btn-lg btn-primary btn-block' type='submit' name='submit'>Submit</button><br />";
 echo "</form></div>";
 ?>
